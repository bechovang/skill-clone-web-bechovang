...
3. **UI primitives** — for every shadcn primitive the specs reference: `npx shadcn@latest add <name>`, then export it from `src/components/ui/index.ts`. Prefer these over custom components everywhere the fidelity target allows.
